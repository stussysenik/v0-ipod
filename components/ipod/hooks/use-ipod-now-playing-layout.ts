...
export function useIpodNowPlayingLayout(options: {
  ...
  useEffect(() => {
    ...
    const handlePointerMove = (event: PointerEvent) => {
      ...
      const maxY = Math.max(24, Math.round((rect.height - options.statusBarHeight) / 2));
      updateLayout(
        dragState.elementId,
        {
          x: clampDragOffset(
    ...
    const handlePointerUp = (event: PointerEvent) => {
      ...
      const finalPosition = getLayoutPosition(layoutRef.current, dragState.elementId);
      console.info("[ipod-os-layout]", {
        phase: "drop",
    ...
    (elementId: IpodNowPlayingLayoutElementId) =>
      (event: React.PointerEvent<HTMLDivElement>) => {
        ...
        dragRef.current = {
          ...
          startPosition: getLayoutPosition(layoutRef.current, elementId),
        };
      },
    [options],
  );
    ...
    (elementId, children, elementOptions) => {
      ...
      return createElement(
        "div",
        {
          ...
          onPointerDown: options.isLayoutMode ? startDrag(elementId) : undefined,
        },
        children,
      );
    },
