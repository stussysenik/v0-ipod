...
import { screenChromeTokens } from "@ipod/lib/support/design-system";
		...
		<div className="w-[min(1080px,94vw)] space-y-8 text-left">
			<section className="space-y-3">
				...
				<p className="max-w-3xl text-sm leading-6 text-black/66">
					These product manifest values feed the current screen frame,
					glass overlay, status divider, play indicator, battery
					chrome, progress footer, and progress track styling in the
					live build.
				</p>
			</section>
			...
			<section className="grid gap-4 lg:grid-cols-3">
				<article className="rounded-[24px] border border-black/10 bg-white/90 p-5 shadow-[0_18px_36px_rgba(0,0,0,0.07)]">
					...
					<TokenLine
						label="liveShadow"
						value={screenChromeTokens.frame.liveShadow}
					/>
					<TokenLine
						label="exportShadow"
						value={screenChromeTokens.frame.exportShadow}
					/>
					<TokenLine
				...
				<article className="rounded-[24px] border border-black/10 bg-white/90 p-5 shadow-[0_18px_36px_rgba(0,0,0,0.07)]">
					...
					<TokenLine
						label="divider"
						value={screenChromeTokens.statusBar.divider}
					/>
					<TokenLine
						label="playIndicator"
					...
					<TokenLine
						...
						value={
							screenChromeTokens.statusBar.battery
								.background
						}
					/>
					<TokenLine
