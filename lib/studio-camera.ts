 ...
 *                 the IG "boomerang" signature. Its turnaround is an authored
 *                 curve carried by the document's time map, not a second easing
 *                 implementation living here.
 *
 * There is no `hold`. A held clip is `repeat: 0` — amplitude zero, resolved in
 * closed form by `lib/motion/transport.ts` — because holding was never a way of
 * threading time through a move, it was the absence of the move.
 */
export type LoopStyle = "loop" | "boomerang";
