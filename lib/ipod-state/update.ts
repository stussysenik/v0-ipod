...
function normalizeModel(model: IpodWorkbenchModel): IpodWorkbenchModel {
  ...
  const rangeEndTime =
    selectionKind === "range"
      ? clampSnapshotTime(
          Math.max(model.playback.rangeEndTime ?? currentTime + 15, rangeStartTime ?? 0),
          duration,
        )
      : null;
        ...
        playback: {
          ...
          rangeStartTime:
            action.payload === "range"
              ? (state.playback.rangeStartTime ?? state.metadata.currentTime)
              : null,
          rangeEndTime:
            action.payload === "range"
              ? (state.playback.rangeEndTime ??
                Math.min(state.metadata.duration, state.metadata.currentTime + 15))
              : null,
        },
      });
    ...
    case "CYCLE_OS_MENU": {
      ...
      return normalizeModel({
        ...
        interaction: {
          ...state.interaction,
          menuIndex:
            (state.interaction.menuIndex + action.payload.direction + total) % total,
        },
      });
    }
