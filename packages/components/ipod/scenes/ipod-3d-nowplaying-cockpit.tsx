...
export function Ipod3DNowPlayingCockpit({
	index,
	metadata,
	dispatch,
}: Ipod3DNowPlayingCockpitProps) {
	return (
			...
			<div className="flex items-center justify-between border-b border-black/[0.06] px-4 py-3.5">
				...
				<div className="flex items-center gap-2.5">
					...
					<EditableTrackNumber
						...
						onTrackNumberChange={(v) =>
							dispatch({
								type: 'UPDATE_TRACK_NUMBER',
								payload: v,
							})
						}
						onTotalTracksChange={(v) =>
							dispatch({
...
function parseTime(value: string): number | null {
	...
	if (trimmed.includes(':')) {
		...
		const mins = Number(m);
		const secs = Number(s);
		if (!Number.isFinite(mins) || !Number.isFinite(secs) || secs >= 60 || secs < 0)
			return null;
