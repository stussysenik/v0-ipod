...
import { deriveOwnedRig, relativeLuminance, STEEL_ROUGHNESS_FLOOR } from './studio-owned-finish';
...
describe('deriveOwnedRig — purity', () => {
	it('never mutates the base config', () => {
		...
		deriveOwnedRig(DESIGNER_DARK_RIG, {
			skin: '#000000',
			back: '#000000',
			stage: '#0b0d12',
		});
...
describe('deriveOwnedRig — I2 separation', () => {
	it('dark device on a dark stage boosts the cool rim', () => {
		const out = deriveOwnedRig(DESIGNER_DARK_RIG, {
			skin: '#0d0f14',
			back: '#0d0f14',
			stage: '#0b0d12',
		});
	...
	it('rim boost grows as the device tone converges on the stage', () => {
		const near = deriveOwnedRig(DESIGNER_DARK_RIG, {
			skin: '#101319',
			back: '#101319',
			stage: '#0b0d12',
		});
		const far = deriveOwnedRig(DESIGNER_DARK_RIG, {
			skin: '#5a5f6b',
			back: '#5a5f6b',
			stage: '#0b0d12',
		});
	...
	it('light device on a light stage deepens the dark contrast panel', () => {
		...
		const out = deriveOwnedRig(APPLE_PRODUCT_RIG, {
			skin: '#fafafa',
			back: '#fafafa',
			stage: '#ffffff',
		});
