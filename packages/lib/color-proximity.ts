...
function rgbToXyz({ r, g, b }: Rgb): { x: number; y: number; z: number } {
	...
	let ng = g / 255;
	let nb = b / 255;
	nr = nr > 0.04045 ? ((nr + 0.055) / 1.055) ** 2.4 : nr / 12.92;
	ng = ng > 0.04045 ? ((ng + 0.055) / 1.055) ** 2.4 : ng / 12.92;
	nb = nb > 0.04045 ? ((nb + 0.055) / 1.055) ** 2.4 : nb / 12.92;
	return {
...
export function deltaECIEDE2000(a: string, b: string): number {
	...
	if (c1Prime * c2Prime === 0) {
		...
		deltaHPrime = 2 * Math.sqrt(c1Prime * c2Prime) * Math.sin((dh * Math.PI) / 180 / 2);
	}
	...
	const rt =
		...
		Math.sin((60 * Math.exp(-(((hBarPrime - 275) / 25) ** 2)) * Math.PI) / 180);
...
export function findNearestShades(baseHex: string, palette: string[], limit = 6): ProximityMatch[] {
	return palette
		.map((hex) => ({
			hex,
...
export function findKumuPaletteProximityMatches(baseHex: string, limit = 8): ProximityMatch[] {
