...
export interface CaptureService {
	readonly prepareNode: (
		element: HTMLElement,
		options: { constrainedFrame: boolean },
	) => Effect.Effect<HTMLElement, Error>;
	readonly captureFrame: (
		...
		threeHandle?: ThreeDIpodHandle | null,
	) => Effect.Effect<ImageBitmap, Error>;
...
export const CaptureServiceLive = Layer.succeed(
	...
	CaptureService.of({
		...
		captureFrame: (node, index, total, plan, backgroundColor, threeHandle) =>
			Effect.tryPromise({
				try: async () => {
					// High fidelity 3D capture if handle is provided
					if (threeHandle) {
						const bitmap = await threeHandle.captureFrame(
							plan.captureWidth,
							plan.captureHeight,
						);
					...
					new Error(`Frame capture failed at index ${index}: ${e}`),
			}),
		cleanupNode: (node: HTMLElement | null) =>
			Effect.sync(() => removeExportNode(node)),
	}),
);
 ...
 */
export const runExportPipeline = (element: HTMLElement, options: ExportOptions) => {
	return Effect.gen(function* (_) {
		...
		const config =
			options.format === 'mp4'
				? MP4_QUALITY_CONFIG[options.quality]
				: GIF_QUALITY_CONFIG[options.quality];
		const fps = config.fps;
		const captureScale = config.scale;

		yield* _(Effect.sync(() => options.onStatusChange('preparing')));
		const exportNode = yield* _(
			capture.prepareNode(element, { constrainedFrame: true }),
		);
		...
		try {
			...
			for (let i = 0; i < plan.frameCount; i++) {
				...
				yield* _(
					Effect.tryPromise({
						try: () =>
							workerClient.call(appendPayload, [
								frame as ImageBitmap,
							]),
						catch: (e) =>
							new Error(
								`Failed to append frame ${i}: ${e}`,
							),
					}),
				);
			...
			const blob = new Blob([finalized.buffer], {
				type:
					finalized.mimeType ||
					(options.format === 'mp4' ? 'video/mp4' : 'image/gif'),
			});
	...
	}).pipe(Effect.provide(CaptureServiceLive), Effect.provide(ExportEnvLive));
};
