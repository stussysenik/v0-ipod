...
describe('export quality tiers', () => {
	...
	it('adds a 60fps cinema MP4 tier with a higher bitrate', () => {
		...
		expect(MP4_QUALITY_CONFIG.cinema.bitrate).toBeGreaterThan(
			MP4_QUALITY_CONFIG.pro.bitrate,
		);
	});

	it('adds a cinema GIF tier', () => {
		expect(GIF_QUALITY_CONFIG.cinema).toBeDefined();
		expect(GIF_QUALITY_CONFIG.cinema.fps).toBeGreaterThanOrEqual(
			GIF_QUALITY_CONFIG.pro.fps,
		);
	});
