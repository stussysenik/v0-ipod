...
describe('stableStringify', () => {
	...
	it('sorts nested keys recursively', () => {
		expect(stableStringify({ o: { y: 1, x: 2 } })).toBe(
			stableStringify({ o: { x: 2, y: 1 } }),
		);
	});
});
...
describe('proofFingerprint', () => {
	...
	it('changes when a pixel-determining input changes', () => {
		...
		expect(
			proofFingerprint(makeProofInputs({ pose: { ...basePose, azimuth: 25 } })),
		).not.toBe(base);
		expect(
			proofFingerprint(
				makeProofInputs({
...
describe('exportFingerprint', () => {
	...
	it("a snapshot's proof key is derivable and stable", () => {
		...
		expect(proofFingerprint(toProofInputs(snap))).toBe(
			proofFingerprint(makeProofInputs()),
		);
	});
});
