...
describe('marquee motion', () => {
	...
	it('loop mode: pauses, then scrolls one full content+gap cycle linearly', () => {
		...
		expect(getMarqueeFrame(metrics, MARQUEE_START_DELAY_MS).translateX).toBeCloseTo(
			0,
			6,
		);

		// Scroll phase is linear in elapsed time (authentic iPod feel).
		const quarter = getMarqueeFrame(
			metrics,
			MARQUEE_START_DELAY_MS + scrollTime * 0.25,
		);
	...
	it('reset mode: scrolls only to reveal the tail, then holds before snapping', () => {
		...
		expect(
			getMarqueeFrame(metrics, cycleDurationMs, 0, 'reset').translateX,
		).toBeCloseTo(0, 6);
	});
