...
import {
	...
	pushSavedColor,
	type SongSnapshot,
} from '@ipod/lib/ipod-state/update';
...
export const ipodCentralMachine = createMachine(
	{
		...
		on: {
			...
			SET_OS_ORIGINAL_MENU_SPLIT: {
				actions: assign(({ context, event }) =>
					normalizeModel({
						...
						interaction: {
							...context.interaction,
							osOriginalMenuSplit: event.payload,
						},
					}),
				),
			},
			SET_OS_NOW_PLAYING_LAYOUT: {
				actions: assign(({ context, event }) =>
					normalizeModel({
						...
						interaction: {
							...context.interaction,
							osNowPlayingLayout: event.payload,
						},
					}),
				),
			},
			SET_OS_NOW_PLAYING_EDITABLE: {
				actions: assign(({ context, event }) =>
					normalizeModel({
						...
						interaction: {
							...context.interaction,
							isNowPlayingEditable: event.payload,
						},
					}),
				),
			},
			...
			TOGGLE_IS_PLAYING: {
				actions: assign(({ context }) =>
					normalizeModel({
						...
						interaction: {
							...context.interaction,
							isPlaying: !context.interaction.isPlaying,
						},
					}),
				),
			},
			...
			APPLY_SONG_SNAPSHOT: {
				actions: assign(({ context, event }) =>
					applySongSnapshotToModel(context, event.payload),
				),
			},
			START_EXPORT: {
				actions: assign({
	...
	},
	{
		actions: {
			...
			incrementTime: assign(({ context }) => {
				const nextTime =
					(context.metadata.currentTime + 1) %
					(context.metadata.duration + 1);
				return normalizeModel({
					...context,
