...
describe.each(IPOD_CLASSIC_PRESETS.map((p) => [p.id, p] as const))(
	...
	(_id, preset) => {
		...
		it('keeps the screen aperture at 52.0 × 39.5 in body proportion', () => {
			expect(
				Math.abs(
					preset.screen.frameWidth / preset.shell.width -
						MM.screen.apertureWidth / MM.body.width,
				),
			).toBeLessThan(RATIO_TOL);
			expect(
				Math.abs(
					preset.screen.frameHeight / preset.shell.height -
						MM.screen.apertureHeight / MM.body.height,
				),
			).toBeLessThan(RATIO_TOL);
		});
		...
		it('preserves the EVEN aperture reveal (top margin ≈ side margin ≈ 4.9mm)', () => {
			...
			expect(Math.abs(preset.shell.paddingTop - sidePx)).toBeLessThan(
				0.1 * pxPerMm,
			);
		});
		...
		it('seats the wheel centre 30.4mm above the bottom edge at Ø38.0', () => {
			expect(
				Math.abs(preset.wheel.size - MM.wheel.diameter * pxPerMm),
			).toBeLessThan(0.2);
			const wheelCenterFromBottom =
				preset.shell.paddingBottom + preset.wheel.size / 2;
			expect(
				Math.abs(
					wheelCenterFromBottom - MM.wheel.centerFromBottom * pxPerMm,
				),
			).toBeLessThan(0.3);
		});
		...
		it('keeps the select button at Ø13.7 in wheel proportion', () => {
			expect(
				Math.abs(
					preset.wheel.centerSize / preset.wheel.size -
						MM.wheel.buttonDiameter / MM.wheel.diameter,
				),
			).toBeLessThan(RATIO_TOL);
		});
		...
		it('projects 1:1 into the 3D scene', () => {
			...
			expect(dims.width / dims.height).toBeCloseTo(
				MM.body.width / MM.body.height,
				3,
			);
			expect(dims.depth / dims.height).toBeCloseTo(
				preset.depthMm / MM.body.height,
				3,
			);
			expect((dims.wheelOuterR * 2) / dims.width).toBeCloseTo(
				MM.wheel.diameter / MM.body.width,
				2,
			);
			// Wheel centre: 30.4mm above the bottom edge, in scene units.
			const centerFromBottom = dims.height / 2 + dims.wheelCenterY;
			expect(centerFromBottom / dims.height).toBeCloseTo(
				MM.wheel.centerFromBottom / MM.body.height,
				2,
			);
...
describe.each(IPOD_CLASSIC_PRESETS.map((p) => [p.id, p] as const))(
	...
	(_id, preset) => {
		...
		it('renders the interactive overlay 1:1 with the 3D wheel mesh', () => {
			const dims = deriveIpod3DDimensions(preset);
			expect(dims.wheelHtmlPx.width * dims.unit).toBeCloseTo(
				dims.wheelOuterR * 2,
				5,
			);
			expect(dims.wheelHtmlPx.height * dims.unit).toBeCloseTo(
				dims.wheelOuterR * 2,
				5,
			);
		});
...
describe('wheel label ink legibility', () => {
	...
	it.each(IPOD_CLASSIC_PRESETS.map((p) => [p.id, p] as const))(
		...
		(_id, preset) => {
			...
			const ring =
				preset.defaultRingColor ??
				deriveWheelColors(preset.defaultShellColor).gradient.via;
...
describe.each(IPOD_CLASSIC_PRESETS.map((p) => [p.id, p] as const))(
	...
	(_id, preset) => {
		...
		it('seats the jack bore Ø3.5 with its centre 8.1mm from the left edge', () => {
			expect(
				Math.abs(
					jack.centerX +
						halfW -
						MM.top.headphoneJack.centerFromLeft * mm,
				),
			).toBeLessThan(SEAT_TOL);
			expect(jack.radius).toBeCloseTo(
				(MM.top.headphoneJack.boreDiameter / 2) * mm,
				5,
			);
		});
		...
		it('keeps the jack rim inside the silhouette (corner-arc dip is sub-hairline)', () => {
			...
			const rimFromEdgeMm =
				MM.top.headphoneJack.centerFromLeft -
				MM.top.headphoneJack.boreDiameter / 2;
		...
		it('spans the hold-switch slot 9.5→20.2mm from the right edge at a 1.8mm slit', () => {
			...
			expect(Math.abs(centerFromRight - ((near + far) / 2) * mm)).toBeLessThan(
				SEAT_TOL,
			);
