...
export type IpodWorkbenchAction =
	...
	| {
			type: 'PATCH_ENV';
			payload: Partial<
				Pick<StudioLightingConfig['env'], 'preset' | 'intensity' | 'blur'>
			>;
	  }
...
export function applySongSnapshotToModel(
	...
	return normalizeModel({
		...
		presentation: {
			...
			edgeColor:
				snapshot.ui.edgeColor ??
				snapshot.ui.backColor ??
				DEFAULT_BACK_COLOR,
	...
	switch (action.type) {
		...
		case 'SET_LIGHTING':
			return patchStudio(state, {
				lighting: cloneLightingConfig(action.payload),
			});
		case 'PATCH_LIGHT':
			return patchStudio(state, {
				lighting: {
			...
			return patchStudio(state, {
				lighting: {
					...
					env: {
						...
						softboxes: state.studio.lighting.env.softboxes.map(
							(box, i) =>
		...
		case 'TOGGLE_INTERACTION_LOCK':
			return patchStudio(state, {
				interactionLocked: !state.studio.interactionLocked,
			});
		case 'SET_MARQUEE':
