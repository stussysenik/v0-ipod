			...
			<div className="flex flex-wrap items-start justify-center gap-16">
				...
				<div className="space-y-4 font-mono text-xs">
					...
					<div className="rounded border border-zinc-700 p-4">
						...
						<p>
							Outer R: {svgOuterR}, Inner R: {svgInnerR}
						</p>
						<p>Ring width: {svgRing}px SVG units</p>
						<p className="text-magenta-400">
							MENU y-center: {svgMenuCenterY} (from top
							edge: {svgMenuFromEdge.toFixed(1)}px ={' '}
							{menuCalPct.toFixed(1)}% of diameter)
						</p>
						<p className="text-cyan-400">
							SKIP x-center: {svgSkipCenterX} (from edge:{' '}
							{svgSkipFromEdge.toFixed(1)}px ={' '}
					...
					<div className="rounded border border-zinc-700 p-4">
						<h2 className="mb-2 font-bold">
							Computed Insets for {w}px wheel
						</h2>
						<p className="text-magenta-400">
							MENU top inset: {px(menuCalPct, w)}px (
							{menuCalPct.toFixed(1)}%)
						</p>
						<p className="text-cyan-400">
							SKIP side inset: {px(skipCalPct, w)}px (
							{skipCalPct.toFixed(1)}%)
						</p>
						<p className="text-magenta-400">
							PAUSE bottom inset: {px(menuCalPct, w)}px (
							{menuCalPct.toFixed(1)}%)
						</p>
					</div>
